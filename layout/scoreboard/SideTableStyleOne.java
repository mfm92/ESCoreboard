...
public class SideTableStyleOne extends SideOverviewTableCreator {
	...
	public Group createSideTable (ArrayList<Participant> finalists) {
	    ...
	    for (int i = 0; i < 6; i++) {
	    	...
	    	Rectangle pointsBase = RectangleBuilder.create()
	    		...
	    		.width(120)
	    		.height(textHeight)
	    		.fill(i%2 == 0 ? bgColorLight : bgColorRed)
	    		.build();
	    	
	    	Text textScore = new Text (finalists.get(i).getScore().get() + "");
	    	textScore.setFont(Font.font("Inconsolata", textScore.getText().length() > 15 ? 26 : 38));
	    	...
	    	Rectangle rectangleEntry = RectangleBuilder.create()
		    	.x(900)
		    	.y(124 + i*textHeight + 60)
		    	.width(480)
		    	.height(40)
		    	.fill(bgColorLighter)
		    	.build();
	    	
	    	Text textEntry = new Text(finalists.get(i).getEntry().getArtist() + " - " + 
		    				finalists.get(i).getEntry().getTitle());
	    	textEntry.setFont(Font.font("Liberation Sans Standard", FontWeight.BOLD, 24));
	    	...
	    	textNation.setFont(Font.font("Coolvetica RG", FontWeight.BOLD, 46));
	    	...
	    	textGroup.getChildren().addAll(rectangle, pointsBase, flag, rectangleEntry, ptsVBox, 
	    			entryVBox, nationVBox);
	    	superText.getChildren().add(textGroup);
	    }
	    
		superText.getChildren().add(RectangleBuilder.create()
				.x(900)
				.y(50)
				.id("Top6Rect")
				.width(600)
