...
public class TwelvePairShowOne extends TwelvePairShower {
	...
	public void addTwelvePair(Scoreboard scoreboard, Participant voter,
		...
		scoreboard.root.getChildren().add(
				RectangleBuilder.create().x(0).y(780).id("12Strip").width(1920)
						.height(270)
						.fill(new ImagePattern(scoreboard.utilities.ppraisbg))
						.build());
		scoreboard.root.getChildren().add(
				RectangleBuilder.create().x(100).y(830).id("12D").width(216)
						.height(170)
		...
		ImageView recFlag = ImageViewBuilder.create().image(receiver.getFlag())
				.x(400).fitHeight(120).fitWidth(230).build();
