...
public class SimpleBannerCreator extends BannerCreator {
	...
	@Override
	public void createBanners (Participant p, String startStatus, String startGrid) {
		Node banner = makeLayout(p, startStatus, startGrid);
		BufferedImage image = new BufferedImage(bannerWidth, bannerHeight, 
				BufferedImage.TYPE_4BYTE_ABGR);
		WritableImage bannerImage = banner.snapshot(new SnapshotParameters(), null);
		BufferedImage bannerNextStep = SwingFXUtils.fromFXImage(bannerImage, image);
		File bannerDestination = new File ("C:\\Users\\MM92\\Desktop\\Study Folder\\"
				+ "Software Engineering\\ESCoreboard\\banners\\" + p.getName().get() + ".png");
	...
	private Node makeLayout (Participant p, String startStatus, String startGrid) {
		...
		partName.setFont(Font.font("Coolvetica RG", partName.getText().length() > 16 ? 28 : 36));
