...
public class Scoreboard extends Application {
    ...
    private synchronized void drawScoreboard(ArrayList<Participant> rosterNations,
    		Standings standings, Stage primaryStage) {
	...
	void update (final ArrayList<Participant> standings, final Participant voter,
    ...
    //NEED A DEEP COPY
    Thread showAndPraise12Pointer (final Participant receiver, final Participant voter,
    		final Standings standings, final int save, final Scoreboard scoreboard) {
    ...
    private void showVideoAndDirect (Participant receiver, final Standings standings, 
    ...
    void generateImageScoreboard (Node rootNode, Participant voter) {
    	...
    	WritableImage scoreboardImage = rootNode.snapshot(new SnapshotParameters(), null);
    	BufferedImage scoreboard = SwingFXUtils.fromFXImage(scoreboardImage, bufferedImage);
    	File destScoreboard = new File("C:\\Users\\MM92\\Desktop\\Study Folder\\"
    	...
    	try {
    		...
    		Logger.getLogger(Scoreboard.class).log(Level.SEVERE, ioex.getMessage());
    	}
    }
}