...
public class Participant implements Comparable<Participant>, Cloneable {
	...
	public SimpleIntegerProperty getScore (ArrayList<Participant> standings) {
		...
		for (Participant p : standings) {
			i++;
			if (p == this) return standings.get(i-1).getScore();
		}
	...
	public void addToScore (int nrOfPointsReceived) {
		...
		switch (nrOfPointsReceived) {
			case 12: receiveHistory[0]++; break;
			case 10: receiveHistory[1]++; break;
			case 8: receiveHistory[2]++; break;
			case 7: receiveHistory[3]++; break;
			case 6: receiveHistory[4]++; break;
			case 5: receiveHistory[5]++; break;
			case 4: receiveHistory[6]++; break;
			case 3: receiveHistory[7]++; break;
			case 2: receiveHistory[8]++; break;
			case 1: receiveHistory[9]++; break;
		}		
	}

	@Override public int compareTo (Participant o) {
		if (o.getScore().get() > score.get()) return 1;
		else if (o.getScore().get() < score.get()) return -1;
		
		if (o.getTotalVoters() > totalVoters) return 1;
		else if (o.getTotalVoters() < totalVoters) return -1;

		for (int i = 0; i < 10; i++) {
			if (o.getReceiveHistory()[i] > receiveHistory[i]) return 1;
			else if (o.getReceiveHistory()[i] < receiveHistory[i]) return -1;
		}
		
		return 0;
	}
	
	@Override public Participant clone() {
